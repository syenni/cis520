...
thread_donate_priority (struct thread *t, int new)
{
	...
	if (t -> priority < new)
		t -> priority = new;
	if (t -> waiting != NULL && t -> waiting -> holder != NULL)
		thread_donate_priority (t -> waiting -> holder, thread_get_priority());
	intr_set_level(old);
}
